...
// UpdatableInbound is implemented by inbounds whose user set can be hot-swapped
// at runtime without restarting the listener. See UpdateUsers.
type UpdatableInbound[T any] interface {
	Inbound
	UpdateUsers(users []T) error
}

// UpdatableShadowsocksInbound is the shadowsocks analogue: it updates users from
// the typed option.ShadowsocksUser list (needed because its update uses the
// password list instead of plain UUIDs).
